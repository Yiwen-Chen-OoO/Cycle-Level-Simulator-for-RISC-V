 ...
 * @LastEditTime: 2019-08-19 16:12:26
...
Core *initCore(Instruction_Memory *i_mem)
{
    ...
    // for the first simulation(task_0)
...
bool tickFunc(Core *core)
{
    ...
    if (ID_EX_Reg.valid)
    {
        ...
        printf("ID_EX--->jal:    %d\n", ID_EX_Reg.jal);
        printf("======================================\n");
        int pipeline_op1 = core->Register[ID_EX_Reg.rs_1];
        int pipeline_op2 = ID_EX_Reg.AluSrc ? ID_EX_Reg.Imm : core->Register[ID_EX_Reg.rs_2];
        ...
        // update PC
        if (ID_EX_Reg.beq && zero)
            core->PC = pipeline_PC - 4 + ID_EX_Reg.Imm;
        else if (ID_EX_Reg.bne && !zero)
            core->PC = pipeline_PC - 4 + ID_EX_Reg.Imm;
        else if (ID_EX_Reg.blt && res)
            core->PC = pipeline_PC - 4 + ID_EX_Reg.Imm;
    ...
    if (EX_MEM_Reg.valid)
    {
        ...
        {
            core->Data_Memory[EX_MEM_Reg.res] = core->Register[EX_MEM_Reg.rs_2];
        }
    ...
    printf("Register x5:    %d\n", core->Register[5]);
    printf("Register x6:    %d\n", core->Register[6]);
    printf("Register x10:    %d\n", core->Register[10]);
    printf("Register x11:    %d\n", core->Register[11]);
